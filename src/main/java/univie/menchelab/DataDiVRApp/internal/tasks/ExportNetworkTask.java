...
import univie.menchelab.DataDiVRApp.internal.util.ConstructJson;
import univie.menchelab.DataDiVRApp.internal.util.ExportContext;
import univie.menchelab.DataDiVRApp.internal.util.Timer;
import univie.menchelab.DataDiVRApp.internal.util.Utility;
...
public class ExportNetworkTask extends AbstractTask implements ObservableTask {
	...
	public File getFileName() {
		if (Objects.isNull(outFile)) {
			File tmp = new File("");
			try {
				tmp = new File(System.getProperty("user.home") + File.separator + "Desktop"
						+ File.separator + "untitled.VRNetz");
			} catch (Exception e) {
	...
	public ExportNetworkTask(CyServiceRegistrar registrar, CyNetwork network) {
	...
	public void writeObject(JSONObject myObject, String fileName) throws IOException {
		...
		System.out.println("Wrote object to " + Paths.get(fileName) + ".");
	};
	...
	public void validatedFileName() {
		...
		// Set Names
		_fileName = getFileName().getAbsolutePath();
		System.out.println("Provided filename:" + _fileName);
